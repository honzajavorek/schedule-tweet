...
if __name__ == '__main__':
    ...
    print('Getting Twitter time')
    twitter_td = get_twitter_time_offset()

    before_test_dt = datetime.now() + twitter_td
    print(f'About to run the code example from README, {before_test_dt.isoformat()} (Twitter time)')
    doctest.testfile('README.rst')
    print(f'The code example has been executed, waiting 4 minutes')
    for i in range(4):
        time.sleep(60)
        print(f'Minute #{i + 1} elapsed')
    after_test_dt = datetime.now() + twitter_td
    print(f'About to do assertions, {after_test_dt.isoformat()} (Twitter time)')

    tweets = read_tweets()
    tweet2 = next(tweets)
    tweet1 = next(tweets)

    print("The tweet #1 starts with 'First Tweet'")
    assert tweet1[0].startswith('First Tweet')
    print("The tweet #1 is within the expected time range")
    assert before_test_dt < tweet1[1] < after_test_dt

    print("The tweet #2 starts with 'Second Tweet'")
    assert tweet2[0].startswith('Second Tweet')
    print("The tweet #1 is within the expected time range")
    assert before_test_dt < tweet2[1] < after_test_dt

    print('The time difference between last two tweets is not more than 1 min')
